...
public interface IUserAddrOrderDao {
    ...
    @Options(useGeneratedKeys = true, keyProperty = "addrOrderId", keyColumn = "addr_order_id")//单条插入返回主键
    int save(UserAddrOrder userAddrOrder);

    @Select("SELECT * FROM tz_user_addr_order where addr_order_id=#{addrOrderId} ")
